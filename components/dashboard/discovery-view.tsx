...
import type { DiscoveryEntry } from "@/lib/can/types";

export function DiscoveryView({ entries, filter, onFilter, onAdd }: { entries: DiscoveryEntry[]; filter: string; onFilter: (value: string) => void; onAdd: (entry: DiscoveryEntry) => void }) {
  const normalized = filter.trim().toLowerCase();
  const visible = entries.filter((entry) => !normalized || `${formatHex(entry.sourceAddress, 2)} ${formatHex(entry.pgn, 5)} ${entry.pgn} ${entry.sourceAddress}`.toLowerCase().includes(normalized));
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[720px] text-left text-sm">
            ...
            <tbody className="divide-y divide-border">
              {visible.map((entry) => <tr key={entry.key} className="hover:bg-accent/35"><td className="px-4 py-3 font-mono text-primary">{formatHex(entry.sourceAddress, 2)}</td><td className="px-4 py-3"><span className="font-mono">{formatHex(entry.pgn, 5)}</span><span className="ml-2 text-xs text-muted-foreground">{entry.pgn}</span></td><td className="px-4 py-3 font-mono text-muted-foreground">{entry.destinationAddress == null ? "Broadcast" : formatHex(entry.destinationAddress, 2)}</td><td className="px-4 py-3 font-mono">{entry.count.toLocaleString()}</td><td className="px-4 py-3 font-mono">{entry.rate.toFixed(1)} Hz</td><td className="px-4 py-3 font-mono text-xs text-muted-foreground">{entry.lastData.map((v) => v.toString(16).padStart(2, "0").toUpperCase()).join(" ")}</td><td className="px-4 py-3 text-right"><Button size="sm" variant="outline" onClick={() => onAdd(entry)}><Plus /> Gauge</Button></td></tr>)}
              {!visible.length && <tr><td colSpan={7} className="px-4 py-16 text-center text-muted-foreground">No matching traffic yet. Start Demo, Replay, or Live CAN.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
